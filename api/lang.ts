...
module f.Lang {
  ...
  /**
  * Interface for the `f.Lang.config` dictionary.
  */
  export interface Config {
    ...
    */
    ...
    /**
    * Tell whether to cache the keys searched for in the `f.Lang.cache` dictionary. <br>
    * The default value is `true`.
    */
    cache: boolean;
  }
  ...
  * The keys are the locale names as defined in the `f.Lang.config.locales` array. <br>
  ...
  export function get( key: string, replacements?: { [key:string]: any } ): string {
    ...
    if ( line === "" ) { 
      ...
      if ( line === "" ) {
        ...
        if ( dico === undefined ) {
          var error = "f.Lang.get(): Dictionary not found for locale '"+locale+"'.";
    ...
    if ( type !== "string" ) {
      ...
      var error = "f.Lang.get(): Provided key '" + key + "' does not lead to a string but to a value of type '" + type + "'."
      console.error( error, "  Value:", line, "  Dictionary:", f.Lang.dictionariesByLocale[ locale ] );
      return error;
    }
  ...
  * @param newCurrentLocale The new current locale name (as set in the `f.Lang.config.locales` array).
  */
  export function update( newCurrentLocale: string ) {
    if ( config.locales.indexOf( newCurrentLocale ) === -1 ) {
      console.error( "f.Lang.update(): Provided new current locale '"+newCurrentLocale+"' is not one of the registered locales.", config.locales );
      return;
    }
